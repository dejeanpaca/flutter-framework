...
class Themes {
  ...
  static int getAppThemeIndex({Brightness? brightness}) {
    ...
    if (themePreference == 'dark') return themeDark;
  ...
  static void applyThemeFromPreference(BuildContext context, {bool callOnApply = true}) {
    ...
    if (kDebugMode) {
      print('theme > Applying from preference: $themePreference, brightness: $brightness, theme: ${theme.name}');
    }
  ...
  static void pickPreference() {
    if (themePreference == 'system') {
      if (lastSystemTheme == 'light' && systemThemes.isNotEmpty && systemThemes[0] != null) {
        setTheme(systemThemes[0]!);
      } else if (lastSystemTheme == 'dark' && systemThemes.length > 1 && systemThemes[1] != null) {
        setTheme(systemThemes[1]!);
      }
    } else {
      if (themePreference == 'light' && themes.isNotEmpty && themes[0] != null) {
        setTheme(themes[0]!);
